...
class CADUrlsTest(TestCase):
    ...
    def test_drawing_details(self):
        # id affected by other test
        res = self.c.get(f'/CAD/drawing/{self.drawing_1.id}/')
        content = res.json()
        # img is a HyperlinkedModelSerializer it will change every time
        #self.assertEquals((content['Title'],content['CAD_file']),('cad1','http://testserver/media/cad_file_EAbX1Ro.png'))

    def test_drawing_series_details(self):
        res = self.c.get(reverse('drawingseries-detail',kwargs={'pk':self.drawing_series_1.id}))
        content = res.json()
        # aerodrome_1 is a HyperlinkedModelSerializer https://stackoverflow.com/questions/39067628/creating-hyperlinked-relations-when-testing-django-rest-framework
        url = 'http://testserver' + reverse('aerodrome-detail', kwargs={'pk': self.aerodrome_1.id})
        self.assertEquals((content['name'],content['Aerodrome']),('series_1',url))
